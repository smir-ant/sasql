...
async fn stream_multiple_rows() {
    let pool = pool().await;
    let mut stream = sasql::query!("SELECT id, login FROM users WHERE active = true ORDER BY id")
        .fetch_stream(&pool)
        .await
        .unwrap();
...
async fn stream_single_row() {
    let pool = pool().await;
    let id = 1i32;
    let mut stream = sasql::query!("SELECT id, login FROM users WHERE id = $id: i32")
        .fetch_stream(&pool)
        .await
        .unwrap();
...
async fn stream_zero_rows() {
    ...
    let mut stream = sasql::query!("SELECT id, login FROM users WHERE login = $login: &str")
        .fetch_stream(&pool)
        .await
        .unwrap();
...
async fn stream_with_nullable_column() {
    let pool = pool().await;
    let mut stream = sasql::query!("SELECT id, middle_name FROM users ORDER BY id")
        .fetch_stream(&pool)
        .await
        .unwrap();
...
async fn stream_connection_returns_to_pool_on_drop() {
    ...
    {
        let mut stream = sasql::query!("SELECT id, login FROM users ORDER BY id")
            .fetch_stream(&pool)
            .await
            .unwrap();
...
async fn stream_collect_all() {
    let pool = pool().await;
    let stream = sasql::query!("SELECT id, login FROM users WHERE active = true ORDER BY id")
        .fetch_stream(&pool)
        .await
        .unwrap();
