...
async fn transaction_commit_persists() {
    ...
    let found = sasql::query!("SELECT id FROM tickets WHERE id = $ticket_id: i32")
        .fetch_optional(&pool)
        .await
        .unwrap();
...
async fn transaction_multiple_queries() {
    ...
    let id1 = t1.id;
    let id2 = t2.id;
    let found1 = sasql::query!("SELECT id FROM tickets WHERE id = $id1: i32")
        .fetch_optional(&pool)
        .await
        .unwrap();
    let found2 = sasql::query!("SELECT id FROM tickets WHERE id = $id2: i32")
        .fetch_optional(&pool)
        .await
        .unwrap();
    ...
    // Clean up.
    sasql::query!("DELETE FROM tickets WHERE id = $id1: i32")
        .execute(&pool)
        .await
        .unwrap();
    sasql::query!("DELETE FROM tickets WHERE id = $id2: i32")
        .execute(&pool)
        .await
        .unwrap();
}
...
async fn transaction_read_your_writes() {
    ...
    let found = sasql::query!("SELECT id, title FROM tickets WHERE id = $ticket_id: i32")
        .fetch_one(&tx)
        .await
        .unwrap();
...
async fn independent_transactions_are_isolated() {
    ...
    let seen = sasql::query!("SELECT id FROM tickets WHERE title = $search: &str")
        .fetch_all(&tx2)
        .await
        .unwrap();
